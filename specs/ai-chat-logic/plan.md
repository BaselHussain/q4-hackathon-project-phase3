...
| V. OpenAI Agents SDK Integration | ✅ PASS | Using official `openai-agents` package with function tools |
...
| VIII. MCP Server Protocol | ✅ PASS | Integrating existing MCP tools as function tools |
...
│   └── agents/
│       └── todo_agent.py     # NEW: Agent setup + instructions
├── tools/
...
| Tool Integration | Function tools via `@function_tool` | Native OpenAI Agents SDK pattern; wraps existing MCP tool functions | Direct MCP protocol (unnecessary indirection) |
...
**Key Decisions**:
1. Use `openai-agents` package with `@function_tool` decorator
2. Wrap existing MCP tool functions as agent tools
...
- Tools: 5 function tools wrapping MCP operations (add, list, complete, delete, update)
...
3. **Agent Layer**
   ...
   - Define function tools wrapping MCP operations
...
| `backend/src/agents/todo_agent.py` | CREATE | Agent setup + tools |
